...
var codeInput = {
  ...
  CodeInput: class extends HTMLElement { // Create code input element
    ...
    update (text) {
      ...
      const result_element = this.querySelector('pre code')
    ...
    sync_scroll () {
      ...
      const input_element = this.querySelector('textarea')
      const result_element = this.template.preElementStyled ? this.querySelector('pre') : this.querySelector('pre code')
      // Get and set x and y
    ...
    check_tab (event) {
      if (this.template.isCode) {
        const input_element = this.querySelector('textarea')
        const code = input_element.value
        if (event.key == 'Tab') {
          /* Tab key pressed */
          event.preventDefault() // stop normal

          if (input_element.selectionStart === input_element.selectionEnd) {
            const before_selection = code.slice(0, input_element.selectionStart) // text before tab
            const after_selection = code.slice(input_element.selectionEnd, input_element.value.length) // text after tab

            const cursor_pos = input_element.selectionEnd + 1 // where cursor moves after tab - moving forward by 1 char to after tab
            ...
            const lines = input_element.value.split('\n')
            let letter_i = 0

            const selection_start = input_element.selectionStart // where cursor moves after tab - moving forward by 1 indent
            const selection_end = input_element.selectionEnd // where cursor moves after tab - moving forward by 1 indent
    ...
    connectedCallback () {
      ...
      const placeholder = this.getAttribute('placeholder') || this.getAttribute('lang') || ''
      ...
      if (this.template.isCode && lang != null) {
    ...
    attributeChangedCallback (name, oldValue, newValue) {
      switch (name) {
        ...
        case 'lang':
          ...
          if (newValue != null) {
            code.className = ('language-' + newValue)
          } else {
            code.className = ''
          }

          if (textarea.placeholder == oldValue) {
  ...
  registerTemplate (template_name, template) {
    // Set default class
  ...
  templates: {
    ...
    }, preElementStyled = true, isCode = true, includeCodeInputInHighlightFunc = false) {
      return {
        highlight,
    ...
    characterLimit () {
      return {
        highlight (result_element, code_input) {
          const character_limit = Number(code_input.getAttribute('data-character-limit'))

          const normal_characters = code_input.escape_html(code_input.value.slice(0, character_limit))
          const overflow_characters = code_input.escape_html(code_input.value.slice(character_limit))
    ...
    rainbowText (rainbow_colors = ['red', 'orangered', 'orange', 'goldenrod', 'gold', 'green', 'darkgreen', 'navy', 'blue', 'magenta'], delimiter = '') {
      return {
        highlight (result_element, code_input) {
          const html_result = []
          const sections = code_input.value.split(code_input.template.delimiter)
